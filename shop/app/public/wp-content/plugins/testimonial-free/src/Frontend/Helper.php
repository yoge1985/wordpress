...
class Helper {
	...
	public static function sp_testimonial_html_show( $post_id, $setting_options, $shortcode_data, $main_section_title ) {
		...
		if ( 'slider' === $layout ) {
			...
			$slider_attr = 'data-swiper=\'{"dots": ' . esc_attr( $pagination ) . ', "spaceBetween": ' . esc_attr( (int) $space['top'] ) . ', "adaptiveHeight": ' . esc_attr( $adaptive_height ) . ', "pauseOnHover": ' . esc_attr( $slider_pause_on_hover ) . ', "slidesToShow": ' . esc_attr( $responsive_columns['large_desktop'] ) . ', "speed": ' . esc_attr( $slider_scroll_speed ) . ', "arrows": ' . esc_attr( $navigation ) . ', "autoplay": ' . esc_attr( $auto_play ) . ', "autoplaySpeed": ' . esc_attr( $slider_auto_play_speed ) . ', "swipe": ' . esc_attr( $slider_swipe ) . ', "swipeToSlide": ' . esc_attr( $swipe_to_slide ) . ', "draggable": ' . esc_attr( $slider_draggable ) . ', "freeMode": ' . esc_attr( $free_mode ) . ', "rtl": ' . esc_attr( $rtl_mode ) . ', "infinite": ' . esc_attr( $slider_infinite ) . ',"slidesPerView": {"lg_desktop":' . $responsive_columns['large_desktop'] . ' , "desktop": ' . $responsive_columns['desktop'] . ', "laptop":' . $responsive_columns['laptop'] . ' , "tablet": ' . $responsive_columns['tablet'] . ', "mobile": ' . $responsive_columns['mobile'] . '},"navigation_mobile": ' . $navigation_mobile . ', "pagination_mobile":' . $pagination_mobile . ', "autoplay_mobile":' . $auto_play_mobile . '}\'';
	...
	public static function testimonial_items( $post_query, $shortcode_data, $post_id, $responsive_columns ) {
		...
		$item_class   = ( 'slider' === $layout ) ? 'swiper-slide' : $grid_columns;
